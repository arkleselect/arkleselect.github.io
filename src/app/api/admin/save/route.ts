...
export async function POST(request: Request) {
    try {
        ...
        if (type === 'post') {
            ...
            if (filename) {
                // Editing mode: Use existing filename
                filePath = path.join(contentDir, 'posts', filename);
            } else {
                // New post mode: Handle collisions
            ...
            const fileName = `${date.replace(/-/g, '').slice(2)}.md`; // YYMMDD format
            ...
            // Daily entries usually append or overwrite?
            // Users often log multiple times a day. Let's append if it exists.
